      ...
      <div className="relative z-10 container mx-auto px-4 py-6 max-w-6xl">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <Link to="/dashboard"><button className="p-2 rounded-full hover:bg-white/60 transition-colors"><ChevronLeft className="w-5 h-5 text-salon-primary" /></button></Link>
          ...
          {activeSection !== "access" && (
            <Button onClick={handleSave} disabled={saving || settingsLoading} className={`rounded-full px-6 font-bold shadow-salon-soft gap-2 transition-all text-white ${saved ? "bg-green-500 hover:bg-green-600" : "bg-salon-gradient hover:opacity-90"}`}>
        ...
        <div className="lg:hidden mb-4 -mx-1">
          <div className="flex gap-2 overflow-x-auto pb-2 px-1 scrollbar-hide">
            {SECTIONS.map((s) => {
              ...
              return (
                <button key={s.id} onClick={() => setActiveSection(s.id)} className={`flex items-center gap-1.5 shrink-0 px-4 py-2.5 rounded-2xl text-sm font-bold transition-all ${active ? "bg-salon-gradient text-white shadow-salon-soft" : "bg-white/70 text-gray-500 border border-salon-soft"}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <aside className="hidden lg:block bg-white/70 backdrop-blur rounded-2xl p-3 shadow-sm border border-white h-fit space-y-1 lg:sticky lg:top-6">
            {SECTIONS.map((s) => {
              const Icon = s.icon;
              const active = activeSection === s.id;
              return (
          ...
          <div className="lg:col-span-3">
            <AnimatePresence mode="wait">
              <motion.div key={activeSection} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.15, ease: [0.25, 0.1, 0.25, 1] }} className="bg-white/70 backdrop-blur rounded-2xl p-6 shadow-sm border border-white">
                {currentSection && (
