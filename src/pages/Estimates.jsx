...
import { db, auth as sbAuth } from "@/api/dataAdapter";  // Phase 5: Supabase
...
function EstimateForm({ estimate, clients, ownerId, priceBookItems, onCancel, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (estimate?.id) {
        await db.entities.Estimate.update(estimate.id, payload);
      } else {
        await db.entities.Estimate.create(payload);
      }
      onSaved();
    } finally {
      setSaving(false);
...
export default function Estimates() {
  ...
  const { data: estimates = [], isLoading } = useQuery({
    queryKey: ["estimates"],
    queryFn: () => db.entities.Estimate.filter({ owner_id: ownerId }),
    enabled: !!ownerId,
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => db.entities.Estimate.delete(id),
